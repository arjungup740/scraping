...
############### Mturk results, first pass at a "ground truth"
ground_truth = pd.read_csv(f'{os.getcwd()}/actor_stuff/first_mturk_results.csv', usecols=['Input.document_url', 'Answer.category.label'])\
				 .rename(columns = {'Input.document_url': 'document_url', 'Answer.category.label':'answer'})
##### take the majority answer as ground truth
ground_truth.loc[ground_truth['answer'] == 'Person did not', 'avg_yes_no'] = 0
ground_truth.loc[ground_truth['answer'] != 'Person did not', 'avg_yes_no'] = 1
bool_ground_truth = ground_truth.groupby('document_url')['avg_yes_no'].mean().reset_index() ## data is bad

len(bool_ground_truth[bool_ground_truth['avg_yes_no'] > .35]) # 310, but adam sandler, adam driver, al pacino, and their pages don't really say that
